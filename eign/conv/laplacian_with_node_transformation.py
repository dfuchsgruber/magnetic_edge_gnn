...
class MagneticEdgeLaplacianWithNodeTransformationConv(nn.Module):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self.node_feature_transformation, "reset_parameters"):
            self.node_feature_transformation.reset_parameters()
        if self.bias is not None:
            nn.init.zeros_(self.bias)
        self._cached_laplacian = None

    def forward(
        self, x: torch.Tensor, edge_index: torch.Tensor, is_directed: torch.Tensor, *args, **kwargs,
    ) -> torch.Tensor:
        if self._cached_laplacian is not None:
            ...
            if self.normalize:
                laplacian, deg_inv_sqrt = degree_normalization(laplacian, return_deg_inv_sqrt=True)
        ...
        if self.q == 0.0:
            ...
            x_nodes = incidence_in @ torch.view_as_complex(x.reshape(-1, self.out_channels // 2, 2))
            x_nodes = self.node_feature_transformation(torch.view_as_real(x_nodes).reshape(-1, self.out_channels), *args, **kwargs)
            x = torch.view_as_real(
                incidence_out.t().conj() @ torch.view_as_complex(x_nodes.reshape(-1, self.out_channels // 2, 2))
